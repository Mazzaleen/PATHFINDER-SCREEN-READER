    ...
    setAndSpeak: function (newElement) {
        if (newElement.style.hidden !== true && newElement.style.visibility !== "none") {
            ...
            if (APPROVED_TAGS.includes(newElement.tagName)) {
                speakThisInternal(newElement.tagName, VOICES[current_voice])
            }
            if (newElement.tagName === "TR") {
                /** speaks a new row instead of "tr" */
                speakThisInternal("Row", VOICES[current_voice])
            }
            ...
            if (newElement.tagName !== "TR") {
                speakThisInternal(newElement.innerText, VOICES[current_voice])
            }
 ...
 * @constructor
 */
const speakThisInternal = (text, voice) => {
...
/*
Method to be called externally by other methods, speaks just the text given
 */
export function speakThisExternal (text) {
    let utter = new SpeechSynthesisUtterance(text);
    utter.voice = VOICES[current_voice]
    utter.rate = voice_speed
    VOICE_SYNTH.speak(utter)
}
...
const progressHandler = (element) => {
    speakThisInternal("Progress Bar", VOICES[current_voice])
    speakThisInternal("Max", VOICES[current_voice])
    speakThisInternal(element.max, VOICES[current_voice])
    speakThisInternal("Progress", VOICES[current_voice])
    speakThisInternal(element.value, VOICES[current_voice])
}
